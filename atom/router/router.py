...
class Router(object):
    def __init__(self, ip, port, apps_dir, run_dir, db_filename):
        self.secure = False
        self.database = adbapi.ConnectionPool('sqlite3', db_filename)
        self.directory = Directory(self)
        self.sessions = SessionManager(self)
        
        deferreds = [self.directory.start(), self.sessions.start()]
        (defer.DeferredList(deferreds, fireOnOneErrback=True)
            .addCallback(self._start_listening, ip, port)
            .addErrback(self._errback))
        
        reactor.run() #@UndefinedVariable
    
    def _start_listening(self, result, ip, port):
    ...
    def _errback(self, failure):
        log.err(failure)
        sys.exit('Error starting server: {}'.format(failure))
